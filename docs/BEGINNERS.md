# ⏱️ Second Thought — BEGINNERS: START HERE (~6 minutes)

**One sentence:** Second Thought looks at a terminal command *before* it runs and stops you if it looks like it will delete everything, leak a password, or break your team's shared work. Spell-check, but for disasters.
...
**Or with one line:** drag **`second-thought`** into any terminal window, type a space and a task word like `test`, press Enter.
...
Task words (every command starts with `./second-thought` + one of these): `test` check-up · `demo` safety drill · `check` judge one command · `setup` turn protection on · `remove` turn it off · `diary` read the log in plain words · `clean` tidy the log. (Older twins still work: doctor=test, reel=demo, init=setup, uninstall=remove. Two names, same jobs — kept so old instructions don't break.)
...
| `.next/` | Leftover from a *different* project that once lived in this folder. Not Second Thought, not a virus. Safe to delete (that project rebuilds it if needed) — or just ignore it. |
...
- `./` means "right here": `./second-thought` only works when the window is already inside this folder. From anywhere else (`/tmp`, home…) you get `no such file or directory ... EXIT:127` — that's the terminal saying "not here," not something broken. Fix: drag `second-thought` into the window instead (that pastes the full location, which works from anywhere), then add your task word.
...
## 3. Check-up: `./second-thought test`

Typical output (your millisecond numbers will differ run to run — normal, not an error):

```text
Second Thought self-test — SAFE: this only reads words and prints opinions. Nothing runs. Cost $0.
...
Verdict: Second Thought works. The only thing that changed: a few diary lines. Nothing else was touched.
```
...
## 4. Judge one command: `./second-thought check "..."`
...
## 5. Drill + diary: `./second-thought demo`
...
**Diary** (`reel/receipt.jsonl` — `.json` = structured text, `.jsonl` = one record per line): skip reading it raw; use `./second-thought diary`:
...
**What "on" looks like:** nearly nothing — until you test it. First, the zero-fear test: `./second-thought demo` (text only, nothing can run). Then the real proof, and here's why it's safe: in a protected terminal your Enter key does NOT run the command — Second Thought sees it *first* and either allows or stops it. So type `ls` → runs. Type `sudo rm -rf /` → red STOP, *nothing happens*. The scary command is the *safest* test, because the check happens before execution, every time. That's on. (Keyless: one reminder line per new terminal until you add a key — one line, not spam.)

Two rules for protected life, especially juniors: a **warning** means stop and read, not "press Enter again" — and **YES is not muscle memory**. YES re-runs a blocked command sight-unseen; if you type it without reading, you've built a one-word self-destruct. Second Thought is a seatbelt, not a replacement for backups, least-privilege accounts, or prod access controls.
